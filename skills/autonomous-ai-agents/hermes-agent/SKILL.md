...
### OAuth Credential Sharing Across Instances

When setting up a second Hermes (e.g., for a friend on a separate Mac mini), the OAuth token can technically be copied from `~/.hermes/auth.json`, but there are important caveats:

- **Same account identity** — both instances appear as the same user to the OAuth provider
- **Concurrent session rate-limiting** — MiniMax (and most providers) may throttle or flag concurrent sessions from the same token
- **Recommendation** — for a friend's独立 Hermes, have them create their own free provider account and run `hermes login` fresh on their machine. This keeps credentials isolated and avoids identity/conflict issues

**Inspecting the credential pool** (to understand what's shared):
```bash
cat ~/.hermes/auth.json | python3 -c "import sys,json; d=json.load(sys.stdin); [print(k, '->', list(v.keys()) if isinstance(v,dict) else type(v).__name__) for k,v in d.items()]"
```

### Reading the Actual Question (Avoid Tangential Over-Explanation)

When Master KT asks a short, specific question (e.g., "disk space?"), do not launch into long context-setups or tangential explanations. Check the actual state first, then return only the answer. Master KT's communication style is: **outcome-only responses, no working steps, no tool output shown, no error logs**. Apply this discipline before volunteering background context. If additional context is genuinely useful, offer it after the core answer — don't lead with it.
...
⚠️ **Auto-delivery pitfall:** When a cron job has `deliver: "origin"` (home channel),
the platform's auto-delivery sends the final response automatically. **Do not**
call `hermes send` or `send_message` for the same target inside the job — the
system deduplicates and will skip the outbound. Put the reminder/notification
text in your final response instead; auto-delivery handles the rest.
See `references/cron-delivery-auto-delivery.md` for the full decision table.
